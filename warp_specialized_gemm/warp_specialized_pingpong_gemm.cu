 ...
 *
 * Two consumer warp groups alternate processing DIFFERENT tiles using TWO
 * separate pipelines to avoid barrier conflicts:
 *   - Pipeline0: Producer ↔ Consumer0 (tiles 0, 2, 4, ...)
 *   - Pipeline1: Producer ↔ Consumer1 (tiles 1, 3, 5, ...)
 *
 * 384 threads total:
 *   WG 0 (threads   0-127) = Producer (TMA loads, interleaves between 2 pipelines)
 *   WG 1 (threads 128-255) = Consumer0 (WGMMA on even tiles via pipeline0)
 *   WG 2 (threads 256-383) = Consumer1 (WGMMA on odd tiles via pipeline1)
 *
 * Key: No producer_tail drain between tiles! Pipeline flows continuously.
 *
 * Build:  make pingpong
...
// Each consumer gets its own smem tile buffers AND pipeline barriers.
// This avoids data races from shared smem stages.
template <class ElementA, class ElementB,
          class SmemLayoutA, class SmemLayoutB, int Stages>
struct SharedStorage
{
  // Consumer0's smem tiles
  alignas(128) cute::ArrayEngine<ElementA, cosize_v<SmemLayoutA>> smem_A0;
  alignas(128) cute::ArrayEngine<ElementB, cosize_v<SmemLayoutB>> smem_B0;
  // Consumer1's smem tiles
  alignas(128) cute::ArrayEngine<ElementA, cosize_v<SmemLayoutA>> smem_A1;
  alignas(128) cute::ArrayEngine<ElementB, cosize_v<SmemLayoutB>> smem_B1;
  // Separate pipeline barriers
  typename cutlass::PipelineTmaAsync<Stages>::SharedStorage pipeline0;
  typename cutlass::PipelineTmaAsync<Stages>::SharedStorage pipeline1;
};
                     ...
                     Alpha alpha, Beta beta, int total_tiles)
{
  ...
  // Separate smem tiles for each consumer to avoid data races
  Tensor sA0 = make_tensor(make_smem_ptr(smem.smem_A0.begin()), SmemLayoutA{});
  Tensor sB0 = make_tensor(make_smem_ptr(smem.smem_B0.begin()), SmemLayoutB{});
  Tensor sA1 = make_tensor(make_smem_ptr(smem.smem_A1.begin()), SmemLayoutA{});
  Tensor sB1 = make_tensor(make_smem_ptr(smem.smem_B1.begin()), SmemLayoutB{});
  ...
  // -----------------------------------------------------------------------
  // Two pipelines: pipeline0 for Consumer0, pipeline1 for Consumer1
  // Each has num_consumers=128 (one warp group)
  // -----------------------------------------------------------------------
  ...
  // Pipeline0: Producer ↔ Consumer0
  typename MainloopPipeline::Params params0;
  params0.transaction_bytes = tma_transaction_bytes;
  params0.num_consumers = cutlass::NumThreadsPerWarpGroup;
  if (is_producer && warp_idx_in_warp_group == 0) {
    params0.role = MainloopPipeline::ThreadCategory::Producer;
  } else if (warp_group_idx == 1) {
    params0.role = MainloopPipeline::ThreadCategory::Consumer;
  } else {
    params0.role = MainloopPipeline::ThreadCategory::NonParticipant;
  }
  params0.is_leader = warp_group_thread_idx == 0;

  // Pipeline1: Producer ↔ Consumer1
  typename MainloopPipeline::Params params1;
  params1.transaction_bytes = tma_transaction_bytes;
  params1.num_consumers = cutlass::NumThreadsPerWarpGroup;
  if (is_producer && warp_idx_in_warp_group == 0) {
    params1.role = MainloopPipeline::ThreadCategory::Producer;
  } else if (warp_group_idx == 2) {
    params1.role = MainloopPipeline::ThreadCategory::Consumer;
  } else {
    params1.role = MainloopPipeline::ThreadCategory::NonParticipant;
  }
  params1.is_leader = warp_group_thread_idx == 0;

  MainloopPipeline pipeline0(smem.pipeline0, params0, Shape<_1, _1, _1>{});
  MainloopPipeline pipeline1(smem.pipeline1, params1, Shape<_1, _1, _1>{});

  __syncthreads();

  // -----------------------------------------------------------------------
  // Each CTA processes 1 tile pair: tile0 (Consumer0) + tile1 (Consumer1)
  // -----------------------------------------------------------------------
  ...
  if (is_producer) {
    // =================================================================
    // PRODUCER: load tile0 via pipeline0, tile1 via pipeline1
    // =================================================================

    cutlass::arch::warpgroup_reg_dealloc<40>();

    if (lane_predicate && warp_idx_in_warp_group == 0) {
      PipelineState pipe0_write = cutlass::make_producer_start_state<MainloopPipeline>();
      PipelineState pipe1_write = cutlass::make_producer_start_state<MainloopPipeline>();

      // Load tile0 via pipeline0 into smem0
      {
        ...
        auto [tAgA, tAsA] = tma_partition(tma_a, Int<0>{}, Layout<_1>{},
                                          group_modes<0,2>(sA0), group_modes<0,2>(gA_t));
        auto [tBgB, tBsB] = tma_partition(tma_b, Int<0>{}, Layout<_1>{},
                                          group_modes<0,2>(sB0), group_modes<0,2>(gB_t));
        for (int k = 0; k < k_tile_count; ++k) {
          pipeline0.producer_acquire(pipe0_write);
          auto* bar = pipeline0.producer_get_barrier(pipe0_write);
          copy(tma_a.with(*bar), tAgA(_, k), tAsA(_, pipe0_write.index()));
          copy(tma_b.with(*bar), tBgB(_, k), tBsB(_, pipe0_write.index()));
          ++pipe0_write;
        }
      }

      // Load tile1 via pipeline1 (no drain needed! uses separate barriers)
      if (has_tile1) {
        auto [mb, nb, valid] = tile_to_coord(tile1_idx);
        Tensor gA_t = local_tile(mA, cta_tiler, make_coord(mb, nb, _), Step<_1, X, _1>{});
        Tensor gB_t = local_tile(mB, cta_tiler, make_coord(mb, nb, _), Step< X, _1, _1>{});
        auto [tAgA, tAsA] = tma_partition(tma_a, Int<0>{}, Layout<_1>{},
                                          group_modes<0,2>(sA1), group_modes<0,2>(gA_t));
        auto [tBgB, tBsB] = tma_partition(tma_b, Int<0>{}, Layout<_1>{},
                                          group_modes<0,2>(sB1), group_modes<0,2>(gB_t));
        for (int k = 0; k < k_tile_count; ++k) {
          pipeline1.producer_acquire(pipe1_write);
          auto* bar = pipeline1.producer_get_barrier(pipe1_write);
          copy(tma_a.with(*bar), tAgA(_, k), tAsA(_, pipe1_write.index()));
          copy(tma_b.with(*bar), tBgB(_, k), tBsB(_, pipe1_write.index()));
          ++pipe1_write;
        }
      }

      // Drain both pipelines
      pipeline0.producer_tail(pipe0_write);
      if (has_tile1) {
        pipeline1.producer_tail(pipe1_write);
      }
    }

  } else {
    // =================================================================
    // CONSUMER0 uses pipeline0, CONSUMER1 uses pipeline1
    // =================================================================

    cutlass::arch::warpgroup_reg_alloc<232>();

    // Select this consumer's pipeline and tile
    MainloopPipeline& my_pipeline = (consumer_idx == 0) ? pipeline0 : pipeline1;
    ...
    if (should_run) {
      ...
      // Each consumer reads from its own smem buffers
      auto& my_sA = (consumer_idx == 0) ? sA0 : sA1;
      auto& my_sB = (consumer_idx == 0) ? sB0 : sB1;
      Tensor tCsA = thr_mma.partition_A(my_sA);
      Tensor tCsB = thr_mma.partition_B(my_sB);
      ...
      // Prologue
      {
        auto bt = my_pipeline.consumer_try_wait(smem_pipe_read);
        my_pipeline.consumer_wait(smem_pipe_read, bt);
      ...
      for (int k = 1; k < k_tile_count; ++k) {
        ...
        warpgroup_fence_operand(tCrC);
        my_pipeline.consumer_release(smem_pipe_release);
        ++smem_pipe_read;
        ++smem_pipe_release;
      }
        ...
        Beta beta, TC* C, int ldC, cudaStream_t stream = 0)
{
  ...
  constexpr int Stages = 3;  // Reduced: 2 pipelines × 3 stages × 32KB = 192KB < 228KB
  ...
  dim3 dimBlock(384); dim3 dimCluster(1,1,1); dim3 dimGrid(grid_blocks,1,1);

  auto* kp = &gemm_device_pingpong<
      decltype(prob_shape), decltype(cta_tiler),
      TA, decltype(sA), decltype(tmaA), TB, decltype(sB), decltype(tmaB),
      TC, decltype(dC), decltype(tiled_mma),
      decltype(alpha), decltype(beta), Stages, Log2Swizzle>;
  ...
  cutlass::launch_kernel_on_cluster(params, (void const*)kp,
      prob_shape, cta_tiler, A, tmaA, B, tmaB, C, dC, tiled_mma, alpha, beta, total_tiles);
  CUTE_CHECK_LAST();
}
...
void cublas_gemm(cublasHandle_t h, int m, int n, int k,
  ...
  cublasHgemm(h,opA,opB,m,n,k,&a16,(__half const*)A,(tA=='N')?m:k,
...
int main(int argc, char** argv) {
  ...
  printf("=== Pingpong Warp-Specialized WGMMA GEMM (Dual Pipeline) ===\n");
  printf("GPU: %s\n\n",props.name);
